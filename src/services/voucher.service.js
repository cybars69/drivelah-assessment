...
class VoucherService {
    ...
    try {
      ...
      if (!data.code) {
        ...
        while (!codeGenerated && attempts < maxAttempts) {
          ...
          const existing = await Voucher.findOne({ code: data.code, deletedAt: null });
          if (!existing) {
            codeGenerated = true;
          }
          attempts++;
  ...
  async findByCode(code) {
    return await Voucher.findOne({ code: code.toUpperCase(), deletedAt: null });
  }

  async findById(id) {
    return await Voucher.findOne({ _id: id, deletedAt: null });
  }
